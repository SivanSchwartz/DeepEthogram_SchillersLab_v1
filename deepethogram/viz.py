...
def plot_confusion_from_logger(logger_file, fig = None, class_names=None, epoch=None):
    ...
    with h5py.File(logger_file, 'r') as f:
        ...
        splits = list(f.keys())
        if 'train' in splits:
            cm_train = f['train/binary_confusion'][epoch, ...].astype(np.int64)
        else:
            cm_train = np.array([np.nan])
        if 'val' in splits:
            cm_val = f['val/binary_confusion'][epoch, ...].astype(np.int64)
        else:
...
# this function is the main that creats all 
def visualize_logger_multilabel_classification(logger_file):
    ...
    # addition to test 
    #fig = plot_confusion_from_logger(logger_file)
    #save_figure(fig, 'Confusion to check', False, 7)
