...
def add_video_to_project(project: dict, path_to_video: Union[str, os.PathLike], mode: str = 'copy') -> str:
    ...
    if os.path.isdir(video_directory):
        print('--------> Video name already exist, change to V2')
        changed_video_directory = video_directory + '_V2'
        os.makedirs(changed_video_directory)
        new_path = os.path.join(changed_video_directory, basename)
        if mode == 'copy':
        ...
        record = parse_subdir(changed_video_directory)
        log.debug('New record after adding: {}'.format(record))
        utils.save_dict_to_yaml(record, os.path.join(changed_video_directory, 'record.yaml'))
        zscore_video(os.path.join(changed_video_directory, basename), project)
